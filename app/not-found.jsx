...
function Error({ statusCode }) {
  return (
    <div className="min-h-screen flex flex-col items-center justify-center">
      ...
      <p className="text-center mt-4 font-light">{getPunchline(statusCode)}</p>
      <Link
        noHighlight
        noExternalLinkIcon
        noGradientUnderline
        href="/"
        className="my-4 sm:my-8 rounded-lg border-[1px] border-tertiary bg-secondary px-4 py-2 flex items-center justify-center"
      >
        {getButtonText(statusCode)}
        {statusCode === 404 && <LuHome className="ml-2" />}
        {statusCode !== 404 && <LuRocket className="ml-2" />}
      </Link>
    </div>
  );
}
